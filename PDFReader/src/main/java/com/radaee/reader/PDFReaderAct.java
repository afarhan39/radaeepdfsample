...
public class PDFReaderAct extends Activity implements OnItemClickListener, OnClickListener, PDFReaderListener, PDFThumbListener
{
    ...
    private void InitView()
    {
		...
		m_thumb.thumbOpen(m_reader.PDFGetDoc(), this, false);
	...
	@Override
	public void OnPageDisplayed(Canvas canvas, PDFVPage vpage) {
		...
		builder.setPositiveButton("OK", new DialogInterface.OnClickListener()
		{
			...
			public void onClick(DialogInterface dialog, int which)
			{
				if( rad_group.getCheckedRadioButtonId() == R.id.rad_copy )
					Toast.makeText(PDFReaderAct.this, getString(R.string.copy_text) + sel_text, Toast.LENGTH_SHORT).show();
				else if( m_reader.PDFCanSave() )
				{
			        ...
			        if( rad_group.getCheckedRadioButtonId() == R.id.rad_copy )
			    	{
	                    ...
	                    android.content.ClipData clip = android.content.ClipData.newPlainText("RadaeePDF SDK", sel_text);
					...
					if( !ret )
						Toast.makeText(PDFReaderAct.this, R.string.annotation_failed, Toast.LENGTH_SHORT).show();
				}
				else
					Toast.makeText(PDFReaderAct.this, R.string.cannot_write_or_encrypted, Toast.LENGTH_SHORT).show();
				onSelect();
				dialog.dismiss();
			}});
		builder.setNegativeButton(R.string.cancel, new DialogInterface.OnClickListener()
